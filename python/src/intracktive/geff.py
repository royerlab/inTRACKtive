...
def geff_to_arrays(zarr_path: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    node_positions = nodes_group['attrs']['position']['values'][:]  # shape: (N, 3) dtype: float16
...
def read_geff_to_df(zarr_path: str) -> pd.DataFrame:
    ...
    edge_ids_int = np.array([[id_mapping[parent], id_mapping[daughter]] for parent, daughter in edge_ids])

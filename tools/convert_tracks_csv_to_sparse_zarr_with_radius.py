...
with open(csv_file, "r") as f:
    ...
    for row in reader:
        t = int(row[1])
        points.append(
            (int(row[0]), t, float(row[2]), float(row[3]), float(row[4]), int(row[5]), float(row[6]), points_in_timepoint[t])
        )
...
points_array = np.ones((timepoints, 4 * max_points_in_timepoint), dtype=np.float32) * -9999.9
...
for point in points:
    ...
    points_array[t, 4 * n:4 * (n + 1)] = [z, y, x, radius]
...
for i, ind in enumerate(tracks_to_points.indices):
    ...
    tracks_to_points_xyz[i] = points_array[t, 4 * n:4 * (n + 1)][:3]

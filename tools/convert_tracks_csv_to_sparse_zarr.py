...
points_to_tracks = dok_matrix((timepoints * max_points_in_timepoint, tracks), dtype=np.int32)
for point in points:
...
points_zarr = zarr.create(
    points_array.shape,
    chunks=tuple(np.ceil(s / 16) for s in points_array.shape),
    dtype=np.float32,
)
points_zarr[:] = points_array
zarr.save(zarr.storage.DirectoryStore(root_dir + "ZSNS001_points.zarr"), points_array)
